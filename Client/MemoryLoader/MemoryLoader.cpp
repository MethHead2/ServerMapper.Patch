...
// Add this function at the top of MemoryLoader.cpp
void EnsureConsoleForMappedCode() {
    // This function ensures the mapped executable can output to console
    if (GetConsoleWindow() == NULL) {
        std::cout << "[MemoryLoader] Allocating console for mapped code..." << std::endl;
        ...
        if (AllocConsole()) {
            ...
            SetConsoleTitleA("Mapped Executable Console");
            std::cout << "Console allocated for mapped executable!" << std::endl;
        }
        else {
            std::cout << "[MemoryLoader] Failed to allocate console. Error: " << GetLastError() << std::endl;
        }
    }
    else {
        std::cout << "[MemoryLoader] Console already available for mapped code" << std::endl;
    }
}
...
bool LoadHeaderlessExecutableFromStream(BIO* bio, HeaderlessExecutable& executable) {
    ...
    void* allocatedBase = GetAllocatedBase();
    if (!allocatedBase) {
    ...
    size_t imageSize = g_handlerInstance->GetImageSize();

    std::cout << "[MemoryLoader] Using pre-allocated base address: 0x"
        << std::hex << executable.baseAddress << std::dec << std::endl;

    // NEW: Ensure console is available for mapped code
    EnsureConsoleForMappedCode();
    ...
    // Store entry point in executable structure
    ...
    // Store sections for later protection setting
    ...
    for (uint32_t i = 0; i < sectionCount; i++) {
        ...
        std::cout << "Section " << i << " VA: " << std::hex << section.virtualAddress
            ...
            << " IsCode: " << section.isCode << std::dec << std::endl;

        // Validation checks
        if (dataSize > 1024 * 1024 * 64) {
        ...
        std::cout << "Section " << i << " data size: " << dataSize << std::endl;
    ...
    for (uint32_t i = 0; i < callbackCount; i++) {
        ...
        std::cout << "TLS callback " << i << " at " << std::hex << callback << std::dec << std::endl;
    ...
    // Send execute info to dummy process via pipe
    std::cout << "Execute: Starting execution in dummy process..." << std::endl;
    std::cout << "Entry point address: " << std::hex << actualEntryPoint << std::dec << std::endl;
    ...
    // Send entry point with architecture-aware handling
    ...
    // Send base address with architecture-aware handling
    if (executable.is32Bit) {
    ...
    // Send auto-execute message
    BYTE autoExecuteMsg = 6;
    ...
    std::cout << "Console output should now work in both processes!" << std::endl;
