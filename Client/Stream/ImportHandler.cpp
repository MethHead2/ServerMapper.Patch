...
bool ImportHandler::CreateDummyProcess(bool is32Bit) {
    ...
    if (!CreatePipe(&hStdOutRead, &hStdOutWrite, &sa, 0)) {
        std::cout << "[ImportHandler] Failed to create stdout pipe. Error: " << GetLastError() << std::endl;
        return false;
    }
    ...
    // Create startup info - MODIFIED TO SHOW WINDOW
    STARTUPINFOA si = { 0 };
    ...
    if (!CreateProcessA(
        ...
        CREATE_NEW_CONSOLE,      // Creation flags - Added CREATE_NEW_CONSOLE for visible console
        NULL,                    // Environment
    ...
    // Read the pipe name from stdout with timeout
    char buffer[1024] = { 0 };
    DWORD bytesRead;
    DWORD totalRead = 0;
    DWORD startTime = GetTickCount();

    // Read until we find a complete line or timeout
    while (totalRead < sizeof(buffer) - 1 && GetTickCount() - startTime < 10000) {
        DWORD available;
        ...
        if (available > 0) {
            DWORD toRead = min(available, sizeof(buffer) - totalRead - 1);
            if (!ReadFile(hStdOutRead, buffer + totalRead, toRead, &bytesRead, NULL)) {
                break;
            }
    ...
    if (totalRead == 0) {
        std::cout << "[ImportHandler] Failed to read pipe name from dummy process" << std::endl;
        CloseHandles();
        return false;
    }

    // Parse the pipe name - should be the first line
    std::string output(buffer, totalRead);
    size_t pos = output.find_first_of("\r\n");
    if (pos != std::string::npos) {
        m_pipeName = output.substr(0, pos);
    }
    else {
        m_pipeName = output;
    }

    std::cout << "[ImportHandler] Received pipe name: " << m_pipeName << std::endl;
