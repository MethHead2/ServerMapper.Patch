...
DWORD CalculateStubSectionSize(size_t importCount, bool is64Bit) {
    // Calculate size based on stub size per import
    DWORD stubSize = is64Bit ? STUB_SIZE_64BIT : STUB_SIZE_32BIT;
    DWORD totalSize = static_cast<DWORD>(importCount * stubSize);
...
DWORD GetStubRVA(const std::vector<BYTE>& peData, size_t importIndex, bool is64Bit) {
    ...
    DWORD stubSize = is64Bit ? STUB_SIZE_64BIT : STUB_SIZE_32BIT;
    return stubSectionRVA + static_cast<DWORD>(importIndex * stubSize);
}
...
bool CreateStubSection(std::vector<BYTE>& peData,
    ...
    try {
        ...
        // Calculate sizes
        DWORD stubSectionSize = CalculateStubSectionSize(imports.size(), is64Bit);
        ...
        stubInfo.stubCount = imports.size();
        ...
        std::cout << "    Stub size per import: " << (is64Bit ? STUB_SIZE_64BIT : STUB_SIZE_32BIT) << " bytes" << std::endl;
