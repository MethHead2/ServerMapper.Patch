...
bool CreateImportStubs(std::vector<BYTE>& peData,
    ...
    try {
        ...
        if (imports.empty()) {
            std::cout << "[*] No imports to create stubs for" << std::endl;
            return true;
        }

        std::cout << "[*] Creating import stubs..." << std::endl;
        std::cout << "    Import count: " << imports.size() << std::endl;
        ...
        // Process each import and create a stub
        int stubsCreated = 0;
        ...
        for (size_t i = 0; i < imports.size(); i++) {
            ...
            ULONGLONG actualFunctionAddress = import.resolvedAddr;

            // Write JMP stub
            BYTE* stubLocation = peData.data() + stubFileOffset;

            if (is64Bit) {
                // 64-bit stub: Use MOV RAX, imm64 + JMP RAX (more reliable than RIP-relative)
                // 48 B8 XX XX XX XX XX XX XX XX - MOV RAX, imm64
                // FF E0                         - JMP RAX

                stubLocation[0] = 0x48;  // REX.W prefix
                stubLocation[1] = 0xB8;  // MOV RAX, imm64
                *reinterpret_cast<ULONGLONG*>(stubLocation + 2) = actualFunctionAddress;
                stubLocation[10] = 0xFF; // JMP RAX
                stubLocation[11] = 0xE0;

                // Fill remaining bytes with INT3 (0xCC) for safety
                for (int j = 12; j < STUB_SIZE_64BIT; j++) {
                    stubLocation[j] = 0xCC;
                }
            }
            else {
